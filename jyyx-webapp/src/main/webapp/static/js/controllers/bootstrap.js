...
/* Controllers */
  ...
  ;
  app.controller('HeadSearchCtrl', ['$scope', '$http','$state', function($scope, $http, $state) {
    $scope.selected = undefined;
    $scope.states = ['分配资源', '扩容/缩容','物理服务器监控','缓存服务监控页面','QPS查询'];
    $scope.statesMap = {"分配资源":"app.allocating", "扩容/缩容":"app.updateMemory", "物理服务器监控":"app.physicalMinitor","缓存服务监控页面":"app.cacheDetail","QPS查询":"app.clusterMonitor"};
    // Any function returning a promise object can be used to load values asynchronously
    $scope.stateGo = function(){
    	$state.go($scope.statesMap[$scope.selected]);
    }
  }])
  ; 
